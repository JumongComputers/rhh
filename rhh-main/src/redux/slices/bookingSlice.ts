import { RoomPrice, RoomPriceType } from './../../types/booking';
...
export const getRoomPrice = createAsyncThunk(
  "booking/getRoomPrice",
  async (_, thunkAPI) => {
    try {
      const response = await bookingService.getRoomPrice();
      return response;
    } catch (error) {
...
export const bookingSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getRoomPrice.pending, (state) =>{
        state.loading = "pending";
      })
      .addCase(getRoomPrice.fulfilled, (state: any, action:any) => {
        state.loading = "succeeded";
        state.bookings = [action.payload]
      })
      .addCase(getRoomPrice.rejected,(state, action)=>{
        state.loading = "failed";
        state.error = action.error.message || null;
      })
