  ...
  return (
    <div className="flex text-center h-[75vh] sm:h-[75vh] md:h-[80vh] lg:h-[50vh] xl:h-[80vh] gap-8 items-center flex-col w-full px-8 py-8 bg-white">
      <div className="relative" onMouseEnter={() => setOverlayVisible(true)} onMouseLeave={() => setOverlayVisible(false)}>
        <img src={data.image} alt={data.roomType} />
        {isOverlayVisible && (
      ...
      <span className="text-[#282d3c] font-bold uppercase text-5xl">{data.roomType}</span>
      ...
      {/* Modal */}
      <BookingModal isOpen={isModalOpen} onRequestClose={closeModal} price={Number(data.price)} roomTypeDefault={data.roomType} />
